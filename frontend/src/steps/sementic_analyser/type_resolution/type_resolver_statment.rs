use models::abstract_syntax_tree::{
    enum_like::UnionVariantKind,
    function::{Function, FunctionSignature},
    objects::{ClassMember, Field},
    soul_type::{GenericDeclare, GenericDeclareKind, NamedTupleType},
    spanned::Spanned,
    statment::{Statement, StatementKind, UseBlock},
};
    ...
    pub(super) fn resolve_statement(&mut self, statment: &mut Statement) {
        match &mut statment.node {
            ...
            StatementKind::Trait(obj) => {
                ...
                for function_signature in &mut obj.methods {
            ...
            StatementKind::Union(obj) => {
                for variant in &mut obj.variants {
                    match &mut variant.node.field {
                        UnionVariantKind::Tuple(items) => {
                            for ty in items {
                                self.resolve_type(ty)
                            }
                        }
                        UnionVariantKind::NamedTuple(items) => {
                            for ty in items.values_mut() {
                                self.resolve_type(ty);
                            }
                        }
                    }
            ...
            StatementKind::Struct(obj) => {
                ...
                for field in &mut obj.fields {
                    self.resolve_field(&mut field.node)
                }
            }
