...
use models::{abstract_syntax_tree::{conditionals::{ElseKind, For, ForPattern, If, While}, expression::{Expression, ExpressionKind}, spanned::Spanned}, error::{SoulError, SoulErrorKind, SoulResult}, soul_names::{KeyWord, TypeModifier}};
...
impl<'a> Parser<'a> {
    ...
    pub(crate) fn parse_while(&mut self) -> SoulResult<Expression> {
        ...
        let condition = if self.current_is(&CURLY_OPEN) {
            ...
            Some(Box::new(
                self.parse_expression(&[CURLY_OPEN])?
            ))
        };
    ...
    fn parse_elses(&mut self) -> SoulResult<Vec<Spanned<ElseKind>>> {
        ...
        loop {
            ...
            if has_else {
                ...
                return Err(
                    SoulError::new(
                        format!("can not have '{ELSE_STR}' or '{ELSE_STR} {IF_STR}' after '{ELSE_STR}'"),
