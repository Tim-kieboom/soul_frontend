...
impl<'a> LlvmBackend<'a> {
    pub(super) fn lower_cast(&self, value: &mir::Operand, cast_to: TypeId) -> SoulResult<IrOperand<'a>> {
        
        let source_operand = self.lower_operand(value)?;
        let cast_type = match self.lower_type(cast_to)? {
            Some(val) => val,
        ...
        match (mir_source_type.kind, mir_cast_type.kind) {
            ...
            (HirTypeKind::Primitive(_), HirTypeKind::Pointer(_)) => {
                // int → ptr
                let (source, cast) = (source_operand.value.into_int_value(), cast_type.into_pointer_type());
                let res = self.builder.build_int_to_ptr(source, cast, "castNumPtr").map_err(build_error)?;
                Ok(IrOperand { value: res.into(), is_signed_interger: false })
            }
            (HirTypeKind::Pointer(_), HirTypeKind::Primitive(_)) => {
                // ptr → int
                let (source, cast) = (source_operand.value.into_pointer_value(), cast_type.into_int_type());
                let res = self.builder.build_ptr_to_int(source, cast, "castPtrNum").map_err(build_error)?;
                Ok(IrOperand { value: res.into(), is_signed_interger: false })
            }
            (HirTypeKind::Primitive(_), HirTypeKind::Primitive(_)) => {
                let info = self.get_primitive_cast_info(mir_source_type, mir_cast_type, source_operand, cast_type)?;
                self.cast_primitives(info)
            }
            _ => Err(soul_error_internal!(format!("types can not be type cast\nsource: {:#?}\ncast: {:#?}", mir_source_type, mir_cast_type), None)),
        }
    ...
    fn int_extend(&self, info: &PrimCastInfo<'a>) -> SoulResult<BasicValueEnum<'a>> {
        ...
        let (source, cast) = (info.source_operand.value.into_int_value(), info.cast_type.into_int_type());
        let res = if info.source_prim.can_be_negative() {
            self.builder.build_int_s_extend(source, cast, "castIntExt").map_err(build_error)?
        } else {
            self.builder.build_int_z_extend(source, cast, "castUintExt").map_err(build_error)?
        };
    ...
    fn int_trunc(&self, info: &PrimCastInfo<'a>) -> SoulResult<BasicValueEnum<'a>> {
        let (source, cast) = (info.source_operand.value.into_int_value(), info.cast_type.into_int_type());
        let res = self.builder.build_int_truncate(source, cast, "castIntTrunc").map_err(build_error)?;
        Ok(res.into())
    }
    ...
    fn int_float_cast(&self, info: &PrimCastInfo<'a>) -> SoulResult<BasicValueEnum<'a>> {
        if info.source_prim.is_float() {
            // float -> int
            let (source, cast) = (info.source_operand.value.into_float_value(), info.cast_type.into_int_type());
            let result = if info.cast_prim.can_be_negative() {
                self.builder.build_float_to_signed_int(source, cast, "castFloatInt")
            } else {
                self.builder.build_float_to_unsigned_int(source, cast, "castFloatUint")
            }.map_err(build_error)?;

            Ok(result.into())
        } else {
            // int -> float
            let (source, cast) = (info.source_operand.value.into_int_value(), info.cast_type.into_float_type());
            let result = if info.source_prim.can_be_negative() {
                self.builder.build_signed_int_to_float(source, cast, "castIntFloat")
            } else {
                self.builder.build_unsigned_int_to_float(source, cast, "castUintFloat")
            }.map_err(build_error)?;
    ...
    fn same_size_cast(&self, info: &PrimCastInfo<'a>) -> SoulResult<BasicValueEnum<'a>> {
        if info.one_is_float {
            self.builder.build_bit_cast(info.source_operand.value, info.cast_type, "castBitcast").map_err(build_error)
        } else {
    ...
    fn float_float_cast(&self, info: &PrimCastInfo<'a>) -> SoulResult<BasicValueEnum<'a>> {
        let (source, cast) = (info.source_operand.value.into_float_value(), info.cast_type.into_float_type());
        let float = if info.source_size < info.cast_size {
            self.builder.build_float_ext(source, cast, "castFloatExt").map_err(build_error)?
        } else {
            self.builder.build_float_trunc(source, cast, "castFloatTrunc").map_err(build_error)?
        };

        Ok(float.into())
    }

    fn get_primitive_cast_info(&self, mir_source_type: &HirType, mir_cast_type: &HirType, source_operand: IrOperand<'a>, destination_type: BasicTypeEnum<'a>) -> SoulResult<PrimCastInfo<'a>> {
