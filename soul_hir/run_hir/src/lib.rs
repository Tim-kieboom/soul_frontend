...
use soul_utils::{compile_options::CompilerOptions, error::SoulError, sementic_level::SementicFault};
...
pub fn to_hir(ast: &AstResponse, options: &CompilerOptions, faults: &mut Vec<SementicFault>) -> HirResponse {
    ...
    if options.debug_view_literal_resolve {
        for value_id in hir.expressions.keys() {
            ...
            if let Some(literal) = literal {
                let msg = SoulError::new(format!("literal resolved to >> {}", literal.value_to_string()), soul_utils::error::SoulErrorKind::Empty, Some(span));
