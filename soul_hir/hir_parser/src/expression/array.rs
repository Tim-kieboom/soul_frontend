use hir::{Assign, ExpressionId, HirType, TypeId};
...
impl<'a> HirContext<'a> {
    ...
    pub(super) fn lower_array(
        ...
        let temp_place = hir::Place::Local(temp_local);
    ...
    fn create_unallocted_array(&mut self, ty: TypeId, element_type: TypeId, size: usize, span: Span) -> ExpressionId {
        let uint = self.add_type(HirType::index_ty());

        let len = self.alloc_expression(span);
        self.insert_expression(len, hir::Expression {
            id: len,
            ty: uint,
        ...
        self.insert_expression(unalloc, hir::Expression {
            ty,
            id: unalloc,
            kind: hir::ExpressionKind::InnerRawStackArray { ty: element_type, len },
        });
        unalloc
    }

    fn create_assign_array_element(&mut self, i: usize, place: &hir::Place, value: ExpressionId, span: Span) -> Assign {
        let ty = self.add_type(HirType::index_ty());
