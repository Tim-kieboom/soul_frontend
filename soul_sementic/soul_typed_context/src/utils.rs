use hir_model::{Body, BodyId, Expression, ExpressionId, FunctionSignature, HirType, HirTypeKind, Primitive};
use parser_models::{ast::{Literal, TypeResult}, scope::NodeId};
use soul_utils::{error::SoulError, sementic_level::SementicFault, soul_names::TypeModifier, span::Span};
...
pub(crate) fn primitive_ty(primitive: Primitive, modifier: Option<TypeModifier>, span: Span) -> HirType {
    HirType { kind: hir_model::HirTypeKind::Primitive(primitive), modifier, span }
}

pub(crate) fn none_ty(span: Span) -> HirType {
    HirType { kind: hir_model::HirTypeKind::None, modifier: None, span }
}

pub(crate) fn bool_ty(modifier: Option<TypeModifier>, span: Span) -> HirType {
    HirType { kind: hir_model::HirTypeKind::Primitive(hir_model::Primitive::Boolean), modifier, span }
}