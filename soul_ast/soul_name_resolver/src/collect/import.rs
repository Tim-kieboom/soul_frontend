...
use ast_model::{AstTree, statements::{ImportKind, ImportPath}};
...
impl<'a> NameResolver<'a> {
    ...
    fn check_if_module_private(&mut self, path: &ImportPath, span: Span) {
        ...
        let is_public = is_name_public(name);
        if !is_public {
    ...
    fn parse_module(
        ...
        let _res = self.scopes.add_module(module_id);
