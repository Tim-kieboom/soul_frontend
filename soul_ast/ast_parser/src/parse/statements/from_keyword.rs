use ast_model::{expression::{Expression, ExpressionKind}, statements::{Statement, StatementKind, TypeDef}};
use soul_tokenizer::model::{TokenKind, keyword::KeyWord};
use soul_utils::{TypeModifier, collections::try_result::{ResultTryErr, ToResult, TryErr, TryOk, TryResult}, error::SoulResult, fault::Fault, soul_error_internal, span::Span};

use crate::{parser::Parser, utils::{ASSIGN, SEMI_COLON, STAMENT_END_TOKENS}};
...
impl<'a, 'f> Parser<'a, 'f> {
    pub(super) fn try_parse_from_keyword(
        ...
        TryOk(match keyword {
            ...
            | KeyWord::Continue => {
                ...
                let expression = Expression::new_id(
                    self.store, 
                    kind, 
            ...
            KeyWord::Pub => {
                ...
                statement.try_set_is_public(true, pub_span.combine(start_span)).try_err()?;
                statement
            }
    ...
    fn parse_type_statement(&mut self, start_span: Span) -> SoulResult<Statement> {
        ...
        Ok(Statement::new(StatementKind::TypeDef(typedef), self.span_combine(start_span)))
    }
}
