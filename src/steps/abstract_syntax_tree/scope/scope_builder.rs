use crate::steps::abstract_syntax_tree::scope::scope::{Scope, ScopeId, TypeSymbol, ValueSymbol};
...
impl ScopeBuilder {
    ...
    /// Resolve a type symbol by walking up the scope tree
    ...
    /// Resolve a value symbol by walking up the scope tree
    ...
    /// Access current scope (immutable)
    pub fn current_scope(&self) -> &Scope {
        &self.scopes[self.current.as_usize()]
    }

    /// Access current scope (mutable)
